...
echo "["$(date +"%y-%m-%d %T")"] Running SPLITREADER 2.5 part 1"  | tee -a $TmpDir/log.txt  
...
if [ -z "$pe" ]
  ...
  /home/alatypova/jdk-21.0.1/bin/java -jar -Xmx10g -Djava.io.tmpdir=$TmpDir/javatemp $picardDir SamToFastq  -INPUT $TmpDir/${in}${ext}.bam -FASTQ $readsDir/$in.fastq -TMP_DIR $TmpDir/javatemp  2>> $TmpDir/log.txt
  ...
  /home/alatypova/jdk-21.0.1/bin/java -jar -Xmx10g -Djava.io.tmpdir=$TmpDir/javatemp $picardDir SamToFastq -INPUT $TmpDir/${in}${ext}.bam -FASTQ $readsDir/$in.1 -SECOND_END_FASTQ $readsDir/$in.2 -TMP_DIR $TmpDir/javatemp  2>> $TmpDir/log.txt
  ...
  samtools view -hF 4 $InputDir/${in}${ext}.bam | awk -v l=$LS '(($4-$8)>(l*10) || ($8-$4)>(l*10) || $7!="=") || $1~/@HD/ || $1~/@SQ/ || $1~/@PG/' | /home/alatypova/jdk-21.0.1/bin/java -jar -Xmx10g -Djava.io.tmpdir=$TmpDir/javatemp $picardDir SamToFastq -INPUT /dev/stdin -FASTQ $readsDir/$in.1b -SECOND_END_FASTQ $readsDir/$in.2b -TMP_DIR $TmpDir/javatemp  2>> $TmpDir/log.txt
...
then
  ...
  bowtie2 -x $TESequences -1 $readsDir/$in.1.fastq -2 $readsDir/$in.2.fastq -S $TmpDir/$in.sam --local --very-sensitive -k $mult_mapping_limit --threads $CORES   2>> $TmpDir/log.txt
fi
...
echo "["$(date +"%y-%m-%d %T")"] Finished running SPLITREADER beta 2.5 part 1"  | tee -a $TmpDir/log.txt  
