...
echo -e "        Pierre Baduell"
echo -e "##############################\n"

# # # List of variable names

workspace_dir='/home/alatypova/workspace_napus'
ref_dir="$workspace_dir/Reference" # path to reference genome annotation files
genome='Darmor'
TE_lib='TE_Darmor_full'
TE_annotation='darmor_dante_ltr_for_insertions_GFF3'
bam_dir="$workspace_dir/BAMs/No2127"
bamext='_dupl_fixed_paired' # extension of bam files
script_dir='/home/alatypova/splitreader_scripts' # location of SPLITREADER scripts
...
#refilt3=1 # force restart of filtering based on negative coverage drop
...
# # # Building reference genome bowtie2 index
    ...
    then
        ...
        ~/jdk-21.0.1/bin/java -jar ~/picard.jar NormalizeFasta -I $genome.fasta -O $genome.norm.fasta
    ...
    do
        ...
        then
            ...
            then
                ...
                echo "SR1 submitted..." 
                $script_dir/SPLITREADER-beta2.5_part1.sh $filename part1 $bam_dir $bamext $cohortname $workspace_dir $TE_lib > $filename.SR1.e
            ...
            then
                ...
                then
                    ...
                    echo "SR2 submitted..." 
                    $script_dir/SPLITREADER-beta2.5_part2.sh $filename $cohortname $workspace_dir Reference/$genome $TE_annotation > $filename.SR2.e
...
then
    ...
    then
        ...
        do
			...
			echo -e $indNB
                    ...
                    for ((l=1; $l<=$end; l=$l+1)); do
                        ...
                        then
                            ...
                            then
                                ...
                                $script_dir/Filter_insertions_splitreader.pl $cohortname $depth $workspace_dir $TE $currdir/$fam/$TE 10 50 $filtname ${SampleNames[*]} > $TE.$cohortname.SRfilt.e
                            else
                    ...
                    if [ $all_TE_finished -gt 0 ]; then
                        ...
                        # # #
			header=$(awk 'NF {print; exit}' "${currdir}/${fam}.${cohortname}-insertions.${filtname}.DP${depth}.bed")
			# header=$(sed -n '5p' "${currdir}/${fam}.${cohortname}-insertions.${filtname}.DP${depth}.bed")
                        # header=$(head -n 1 "${currdir}/${fam}.${cohortname}-insertions.${filtname}.DP${depth}.bed")
			echo "header is $header"
			# echo $(head -n 5 "${currdir}/${fam}.${cohortname}-insertions.${filtname}.DP${depth}.bed")
        ...
        then
			...
			echo -e $rc_size_up
			echo -e $rc_size_down
                        if [[ $rc_size -ge $custom_size && $rc_size_up -ge $custom_size && $rc_size_down -ge $custom_size ]]
                        then 
                    ...
                    then
                        ...
                        $script_dir/BAM-readcount_wrapper.sh $filename $bam_dir $bamext $filename $depth $cohortname $curr_dir $workspace_dir/BEDfiles/SPLITREADER/ $ref_dir/ $genome.fasta > $filename.BAMrc.e
                    fi
                fi
            done
    ...
    if [ $all_ind_finished -eq 1 ]; then
        ...
        if [ $refilt3 -gt 0 ]; then
            ...
            rm $cohortname-insertions.ratioNC$filtname*.DP$depth.bed
	...
	if [ ! -e $cohortname-insertions.ratioNC$filtname.NConly.DP$depth.bed ]
        then
            ...
            $script_dir/Filter_negative_calls_splitreader.pl $cohortname $depth $curr_dir $workspace_dir $filtname ${SampleNames[*]}
