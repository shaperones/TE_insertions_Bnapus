...
# bam-readcount executable path
BRDir=/home/alatypova/bam-readcount/build/bin

# Temporary directory
TmpDir=$project_dir/localtmp/QD-$popname
mkdir -p $TmpDir
...
then
    ...
    $BRDir/bam-readcount -w 1 -f $GenomeIndexDir/$genomefile $bamdir/${in}$bamext.bam -l $OutputDir/$cohort-insertion-sites.0.v2.bed > $OutputDir/$popname.$cohort-insertion.tmp.0.v2.rc
    # # process rc counts to keep only min DP over each insertion
    perl /home/alatypova/splitreader_scripts/Process_BAMrc_splitreader.pl $cohort $depth $popname $OutputDir 0.v2
  ...
  ## calculate coverage over insertions
  $BRDir/bam-readcount -w 1 -f $GenomeIndexDir/$genomefile $bamdir/${in}$bamext.bam -l $OutputDir/$cohort-insertion-sites.0.bed > $OutputDir/$popname.$cohort-insertion.tmp.0.rc
  # # process rc counts to keep only min DP over each insertion
  perl /home/alatypova/splitreader_scripts/Process_BAMrc_splitreader.pl $cohort $depth $popname $OutputDir 0
...
then
  ...
  then
      ...
      $BRDir/bam-readcount -w 1 -f $GenomeIndexDir/$genomefile $bamdir/${in}$bamext.bam -l $OutputDir/$cohort-insertion-sites.100up.v2.bed > $OutputDir/$popname.$cohort-insertion.tmp.100up.v2.rc
      perl /home/alatypova/splitreader_scripts/Process_BAMrc_splitreader.pl $cohort $depth $popname $OutputDir 100up.v2
    ...
    $BRDir/bam-readcount -w 1 -f $GenomeIndexDir/$genomefile $bamdir/${in}$bamext.bam -l $OutputDir/$cohort-insertion-sites.100up.bed > $OutputDir/$popname.$cohort-insertion.tmp.100up.rc
    # # # process rc counts to keep only min DP over each insertion
    perl /home/alatypova/splitreader_scripts/Process_BAMrc_splitreader.pl $cohort $depth $popname $OutputDir 100up
  ...
  then
      ...
      $BRDir/bam-readcount -w 1 -f $GenomeIndexDir/$genomefile $bamdir/${in}$bamext.bam -l $OutputDir/$cohort-insertion-sites.100down.v2.bed > $OutputDir/$popname.$cohort-insertion.tmp.100down.v2.rc
      perl /home/alatypova/splitreader_scripts/Process_BAMrc_splitreader.pl $cohort $depth $popname $OutputDir 100down.v2
    ...
    $BRDir/bam-readcount -w 1 -f $GenomeIndexDir/$genomefile $bamdir/${in}$bamext.bam -l $OutputDir/$cohort-insertion-sites.100down.bed > $OutputDir/$popname.$cohort-insertion.tmp.100down.rc
    # # process rc counts to keep only min DP over each insertion
    perl /home/alatypova/splitreader_scripts/Process_BAMrc_splitreader.pl $cohort $depth $popname $OutputDir 100down
