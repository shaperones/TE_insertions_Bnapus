...
open IN, "<$project_dir/$subsetname-insertions.$filtname.DP$depth.header.bed" ;
while(<IN>){
	chomp $_ ;
	...
	if(scalar(@line)>2){
		...
		# print STDOUT $scaff, "\t", $pos_start, "\t",  $pos_end, "\n";
...
while(<IN>){
	...
	if ($masked_pos{$scaff}{$pos_start}{$pos_end}){
		...
		foreach my  $pop (sort keys %ind_pop){
			...
			# print STDOUT $scaff, "\t", $pos_start, "\t",  $pos_end, "\t", $TE_name, "\t", $pop, "\t",  $line[$ind_pop{$pop}], "\n";
			}
		}
	}
}
	...
	foreach my $posstart (sort{$a<=>$b} keys %{$TE_hash{$scf}} ){
		foreach my $posstop (sort{$a<=>$b} keys %{$TE_hash{$scf}{$posstart}} ){
			foreach my  $pop (sort keys %ind_pop){
				...
				if($size>1){
					print STDOUT $scf, "\t", $posstart, "\t", $posstop, "\t", $pop, "\t";
	...
	foreach my $posstart (sort{$a<=>$b} keys %{$TE_cov{$scf}} ){
		foreach my $posstop (sort{$a<=>$b} keys %{$TE_cov{$scf}{$posstart}} ){
			foreach my $TE (keys %{$TE_cov{$scf}{$posstart}{$posstop}} ){
				# print STDOUT "check dup, $scf, $posstart, $posstop, $TE,  $TE_cov{$scf}{$posstart}{$posstop}{$TE}";
				if ($TE_cov{$scf}{$posstart}{$posstop}{$TE}{'dup'}<1){ # check if site is not a duplicate
					...
					if($site_filter_bool>=1 && $NC_filter_nb>=$ind_thresh){
						# print STDOUT "at least 1 TRUE POSITIVEs and at least 10% inds covered\t$site_filter_bool\t$NC_filter_nb\t$ind_thresh\n";
