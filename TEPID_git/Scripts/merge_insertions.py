...
def overlap(start1, stop1, start2, stop2):
    ...
    for y in range(start2, stop2+100):
        if start1 <= y <= stop1:
            return True
        else:
            pass
...
#            pass
    ...
    for file_name in os.listdir('.'):
        #print(file_name)
        if os.path.isfile(file_name) is True and file_name.startswith(filename):
               # print("True")
                try:
                    master_insertions
                except NameError:
                    master_insertions = create_master_dict(file_name, file_name[11:-4])
                else:
                    merge_insertions(master_insertions, file_name, file_name[11:-4])
    print(os.getcwd())
...
main(options.filename, options.dir)
