...
def _overlap(start1, stop1, start2, stop2, d=0):
    ...
    """
    d = int(d)
    for y in range(start2-d, stop2+d):
        if start1 <= y <= stop1:
            return True
        else:
            pass
...
def check_bam(bam, p, prefix, make_new_index=False):
    ...
    try:
        ...
        pysam.sort('-@', p, '-o', prefix + 'sorted.temp', bam)
        os.remove(bam)
        os.rename(prefix + 'sorted.temp', bam)
    else:
    ...
    with open(inp, 'r') as infile, open('deletions_{}.bed'.format(acc), 'w+') as outfile, open('deletion_reads_{}.txt'.format(acc), 'w+') as deletions_reads:
        for line in infile:
            ...
            if (gapsize <= 0) or (name in written_tes) or ((length-mn) > gapsize):
                ...
                if name not in list(tes.keys()):  # first time seeing this TE, add to dict
...
def check_name_sorted(bam, p, prefix):
    ...
    try:
        ...
        print('  sorting bam file')
        pysam.sort('-@', p, '-n', '-o', prefix + 'sorted.temp', bam)
        os.remove(bam)
        os.rename(prefix + 'sorted.temp', bam)
    else:
...
def discover_pe(options):
    ...
    if options.discordant is False:
        ...
        pysam.sort('-@', str(options.proc), '-n','-o', options.prefix + 'disc_sorted', options.prefix + 'disc_bam.temp')
        disc = pybedtools.BedTool(options.prefix + 'disc_sorted')\
...
def remove_temp(options):
    if options.keep is False:
        ...
        for i in mytempfiles:
            os.remove(i)
        os.remove(options.prefix + 'disc_sorted')
